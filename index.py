...
@APP.route("/songdb/<VERSION>/songs", methods=['GET'])
@authorization_required
def songdb(VERSION):
    if VERSION == "v2" or "v1":
        return errorResponse("This version is not supported for your X-SkuId", 404)
    ...
    for SONG in DATABASE.getSongs(XSKUID):
        CONTENT[SONG['mapName']] = {
            "artist": SONG['artist'],
            'assets': {},
        ...
        for key, val in configuration.ASSETS.items():
            ...
            for object in STORAGE.getFiles():
                if val in object:
                    CONTENT[SONG['mapName']]['assets'][key] = object
                    break
        ...
        for key in configuration.URLS:
            ...
            for object in STORAGE.getFiles():
                if file in object:
                    CONTENT[SONG['mapName']]['urls'][key] = object
                    break
